...
GEMINI_WEB_TRANSPORT = "gemini-web"
ORDINARY_GEMINI_PRODUCT_SEMANTICS = "ordinary-gemini"

_GEMINI_CAPABILITY_STATES: dict[str, CapabilityState] = {
...
class GeminiWebWriteOutcomeAmbiguousError(RequestError):
    """Post-submit Gemini outcome that must never authorize automatic replay."""

    reconciliation_required = True
    automatic_retry_allowed = False


class GeminiBrowserTurnProvider(BrowserNativeTurnProvider):
    """Experimental Gemini Web turn provider over the existing local bridge."""
    ...
    def _conversation_id(value: Any) -> str | None:
        ...
        if not isinstance(value, str) or not value.strip():
            raise ValueError("Gemini conversation_id must be a non-empty opaque id")
        return value.strip()
        ...
        *,
    ...
    ) -> GeminiBrowserTurnResult:
        if not isinstance(text, str) or not text.strip():
            raise ValueError("text is required")
        if len(text) > 200_000:
            raise ValueError("text is too large for Gemini Web turn")
        ...
        if response.get("ok") is not True:
            ...
            if error.startswith(
                ...
                raise GeminiWebWriteOutcomeAmbiguousError(
                    f"GEMINI_WEB_TURN_FAILED: {error}",
                    request_stage="gemini_web_turn",
                )
            raise RequestError(
                f"GEMINI_WEB_TURN_FAILED: {error}",
                request_stage="gemini_web_turn",
            )
        if response.get("providerId") != GEMINI_PROVIDER_ID:
            raise RequestError(
                "GEMINI_WEB_PROVIDER_ID_MISMATCH",
                request_stage="gemini_web_turn",
            )
        ...
        if not isinstance(resolved_id, str) or not resolved_id.strip():
            raise RequestError(
                "GEMINI_WEB_CONVERSATION_ID_MISSING",
                request_stage="gemini_web_turn",
            )
        if not isinstance(response_text, str) or not response_text.strip():
            raise RequestError(
                "GEMINI_WEB_RESPONSE_TEXT_MISSING",
                request_stage="gemini_web_turn",
            )
        if not isinstance(final_url, str) or not (
            final_url == "https://gemini.google.com"
            or final_url.startswith("https://gemini.google.com/")
        ):
            raise RequestError(
                "GEMINI_WEB_FINAL_ROUTE_INVALID",
                request_stage="gemini_web_turn",
            )
        if response.get("finalityEvidence") != "PAGE_DOM_STABLE_COMPLETION":
            raise RequestError(
                "GEMINI_WEB_FINALITY_UNPROVEN",
                request_stage="gemini_web_turn",
            )
        if response.get("canonicalCompletionProven") is not False:
            raise RequestError(
                "GEMINI_WEB_CANONICAL_FINALITY_MUST_REMAIN_UNPROVEN",
                request_stage="gemini_web_turn",
            )
        if response.get("routeIdentityProven") is not True:
            raise RequestError(
                "GEMINI_WEB_ROUTE_IDENTITY_UNPROVEN",
                request_stage="gemini_web_turn",
            )
        if response.get("automaticWriteRetry") is not False:
            raise RequestError(
                "GEMINI_WEB_AUTOMATIC_RETRY_FORBIDDEN",
                request_stage="gemini_web_turn",
            )

        return GeminiBrowserTurnResult(
...
def _gemini_capabilities() -> ProductCapabilities:
    return ProductCapabilities.from_entries(
        transport=GEMINI_WEB_TRANSPORT,
        product_semantics=ORDINARY_GEMINI_PRODUCT_SEMANTICS,
        entries=tuple(
            ProductCapability(
                name=name,
                state=_GEMINI_CAPABILITY_STATES[name],
                owner=CapabilityOwner.TRANSPORT,
                evidence=(
                    "PR15.55 minimal Gemini Web text new-chat/continuation proof"
                    if name in {TEXT_TURNS, NEW_CHAT, CONTINUATION}
                    else "outside PR15.55 minimal Gemini Web proof"
                ),
            )
...
class GeminiWebTransport:
    transport_id = GEMINI_WEB_TRANSPORT

    def __init__(self, provider: GeminiBrowserTurnProvider | None = None) -> None:
        self.provider = provider or GeminiBrowserTurnProvider()
    ...
    def health(self, conversation: Any = None) -> ProductRuntimeHealth:
        ...
        return ProductRuntimeHealth(
            ...
            conversation_id=(
                GeminiBrowserTurnProvider._conversation_id(conversation)
                if conversation is not None
                else None
            ),
            canonical_status=None,
            canonical_read_checked=False,
            read_plane="GEMINI_PAGE_DOM",
            session_plane="GEMINI_BROWSER_SESSION",
            write_plane="GEMINI_PAGE_DOM_WRITE",
            automatic_write_retry=False,
    ...
    def send_text_observed(
        ...
        *,
        ...
        provenance = ProductExecutionProvenance(
            ...
            transport=self.transport_id,
            write_plane="GEMINI_PAGE_DOM_WRITE",
            readback_plane="GEMINI_PAGE_DOM",
            session_plane="GEMINI_BROWSER_SESSION",
...
class GeminiWebRuntime:
    ...
    def __init__(self, write_transport: GeminiWebTransport | None = None) -> None:
        self.write_transport = write_transport or GeminiWebTransport()
