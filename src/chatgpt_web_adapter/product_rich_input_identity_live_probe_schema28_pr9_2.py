...
from .product_rich_input_live_gate_schema28_pr9_2 import (
    ProductRichInputSchema28LiveProvider,
    _validate_support,
)
...
def run_identity_probe(*, timeout: float = 150.0) -> dict[str, Any]:
    """Perform exactly one authenticated rich new-chat write to prove schema-28 identity."""
    if timeout <= 0:
    ...
    report: dict[str, Any] = {
        ...
        "pr": "PR9.2",
        "schema": SCHEMA,
        "probe": "SCHEMA28_REQUEST_BOUND_IDENTITY",
    ...
    with tempfile.TemporaryDirectory(prefix="cwa-pr9-2-schema28-identity-live-") as temp_dir:
        ...
        turn = _v7._validate_execution(
            label="SCHEMA28_IDENTITY_IMAGE_NEW_CHAT",
            execution=execution,
            events=events,
        ...
        if not isinstance(conversation_id, str) or not conversation_id.strip():
            raise RuntimeError("PR9_2_SCHEMA28_IDENTITY_PROBE_CONVERSATION_ID_NOT_PROVEN")
        report["turn"] = turn

    if report["write_attempts"] != PRODUCT_WRITE_BUDGET:
        raise RuntimeError("PR9_2_SCHEMA28_IDENTITY_PROBE_WRITE_BUDGET_MISMATCH")
    if report["write_completions"] != PRODUCT_WRITE_BUDGET:
        raise RuntimeError("PR9_2_SCHEMA28_IDENTITY_PROBE_WRITE_COMPLETION_COUNT_MISMATCH")
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description=(
            "PR9.2 schema-28 one-write authenticated request-bound identity probe"
        )
    )
