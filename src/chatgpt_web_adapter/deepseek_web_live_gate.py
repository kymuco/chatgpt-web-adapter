...
from .deepseek_web import DeepSeekWebRuntime
...
def run_live_gate(*, timeout: float = 150.0) -> dict[str, object]:
    ...
    if not health.ready:
        raise RuntimeError(
            "DEEPSEEK_WEB_LIVE_GATE_NOT_READY: install/reload the browser-native "
            "extension, keep its Native Messaging host running, and sign in to "
            "https://chat.deepseek.com in that Chrome profile"
        )

    first_marker = f"DS-FIRST-{uuid.uuid4().hex[:12]}"
    second_marker = f"DS-CONT-{uuid.uuid4().hex[:12]}"
    ...
    if not isinstance(conversation_id, str) or not conversation_id:
        raise RuntimeError("DEEPSEEK_WEB_LIVE_GATE_NEW_CHAT_IDENTITY_MISSING")
    if first_marker not in first.text:
        raise RuntimeError("DEEPSEEK_WEB_LIVE_GATE_NEW_CHAT_TEXT_MISMATCH")
    ...
    if second.conversation.conversation_id != conversation_id:
        raise RuntimeError("DEEPSEEK_WEB_LIVE_GATE_CONTINUATION_IDENTITY_CHANGED")
    if second_marker not in second.text:
        raise RuntimeError("DEEPSEEK_WEB_LIVE_GATE_CONTINUATION_TEXT_MISMATCH")
