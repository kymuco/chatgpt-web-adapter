...
from .google_translate_web_pr17_0 import GoogleTranslateWebRuntime
...
def main() -> int:
    ...
    if "casa" not in first_normalized or "azul" not in first_normalized:
        raise RuntimeError(
            "GOOGLE_TRANSLATE_FIRST_SEMANTIC_CHECK_FAILED: "
            f"{first.text!r}"
        )
    ...
    for token in ("eins", "zwei", "drei"):
        if token not in second_normalized:
            raise RuntimeError(
                "GOOGLE_TRANSLATE_SECOND_SEMANTIC_CHECK_FAILED: "
                f"{second.text!r}"
            )
