...
class GoogleTranslateWebRuntime:
    ...
    not accepted by ProductProviderBoundary schema 2. The translation proof
    tests reuse of browser-owned safety mechanics without pretending to have
    chat/conversation semantics.
    """
    ...
    def _normalize_language(value: str, *, source: bool) -> str:
        ...
        if not all(piece.isalnum() and 2 <= len(piece) <= 8 for piece in pieces):
    ...
    def translate(
        ...
        *,
        ...
        if len(text) > 5000:
            raise ValueError(
                "Google Translate Web text input is limited to 5000 characters"
            )
        ...
        try:
            response = self._bridge._rpc(  # noqa: SLF001 - internal bridge reuse
                {
                    "type": "translate_text",
