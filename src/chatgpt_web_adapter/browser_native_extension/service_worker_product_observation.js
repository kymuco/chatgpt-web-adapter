// PR15.22 explicit product-message observation owner.
//
// Consolidates connector lifecycle evidence, connector-router characterization,
// and generated-artifact observation into one shipping owner. Each observation
// layer remains evidence-only and calls the lower layer first, preserving the
// historical source-order semantics without runtime monkeypatch composition.
...
function _pr100ConnectorInspectMessageLayer(context, state, message, next) {
  next(context, state, message);
  try {
...
function _pr100RouterInspectMessageLayer(context, state, message, next) {
  next(context, state, message);
  try {
...
function _pr101GeneratedArtifactInspectMessageLayer(context, state, message, next) {
  next(context, state, message);
  try {
...
// Historical final wrapper order was generated artifact -> connector router ->
// connector lifecycle -> PR8.12 base inspection.
const _pr10ProductObservationUpstreamInspectMessage = _pr812InspectMessage;

function _pr10ProductInspectMessageOwner(context, state, message) {
  return _pr101GeneratedArtifactInspectMessageLayer(
    context,
    state,
    message,
    (artifactContext, artifactState, artifactMessage) =>
      _pr100RouterInspectMessageLayer(
        artifactContext,
        artifactState,
        artifactMessage,
        (routerContext, routerState, routerMessage) =>
          _pr100ConnectorInspectMessageLayer(
            routerContext,
            routerState,
            routerMessage,
            _pr10ProductObservationUpstreamInspectMessage
          )
      )
  );
}
