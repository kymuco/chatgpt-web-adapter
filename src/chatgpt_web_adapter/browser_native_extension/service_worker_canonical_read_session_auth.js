const _cwaCanonicalFetchWithoutSessionBearer = _cwaCanonicalFetch;
const CWA_CANONICAL_SESSION_AUTH_PATCH_KEY = "__cwaCanonicalSessionAuthPatch";
const CWA_CANONICAL_SESSION_MAX_CHARS = 262_144;
const CWA_CANONICAL_ACCESS_TOKEN_MAX_CHARS = 100_000;

function _cwaCanonicalSessionAuthFailure(status, contentType, reasonCode) {
  return {
    ok: false,
...
async function _cwaCanonicalSessionAuthEvaluateInRuntimeTab(tabId, expression) {
...
async function _cwaInstallCanonicalSessionBearer(
  tabId,
  conversationId,
  timeoutMs
) {
  ...
  const expression = `(async () => {
    const stateKey = ${JSON.stringify(CWA_CANONICAL_SESSION_AUTH_PATCH_KEY)};
    ...
    const patchedFetch = function cwaCanonicalAuthorizedFetch(input, init = {}) {
      let url;
      try {
  ...
  return _cwaCanonicalSessionAuthEvaluateInRuntimeTab(tabId, expression);
}

async function _cwaRestoreCanonicalSessionBearer(tabId) {
  const expression = `(() => {
    const stateKey = ${JSON.stringify(CWA_CANONICAL_SESSION_AUTH_PATCH_KEY)};
  ...
  const result = await _cwaCanonicalSessionAuthEvaluateInRuntimeTab(
    tabId,
    expression
  );
...
_cwaCanonicalFetch = async function _cwaCanonicalFetchWithSessionBearer(
  tabId,
  conversationId,
  timeoutMs,
  includeAllPages
) {
  const installed = await _cwaInstallCanonicalSessionBearer(
    tabId,
    conversationId,
    timeoutMs
  );
  if (installed.ok !== true) {
    return _cwaCanonicalSessionAuthFailure(
      installed.status,
  ...
  try {
    return await _cwaCanonicalFetchWithoutSessionBearer(
      tabId,
      conversationId,
      timeoutMs,
      includeAllPages
    ...
    await _cwaRestoreCanonicalSessionBearer(tabId);
  }
};
