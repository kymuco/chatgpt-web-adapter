...
const CWA_GEMINI_PROVIDER_ID = "gemini";
const CWA_GEMINI_ORIGIN = "https://gemini.google.com";
const CWA_GEMINI_RUNTIME_TAB_KEY = "geminiWebRuntimeTabIdV1";
const CWA_GEMINI_ROUTE_MAP_KEY = "geminiWebConversationRoutesV1";
const CWA_GEMINI_STABLE_MS = 9000;

function _cwaGeminiIsUrl(url) {
  try {
    return new URL(url).origin === CWA_GEMINI_ORIGIN;
  } catch {
    return false;
  }
}

async function _cwaGeminiStorageGet(key) {
  const result = await chrome.storage.local.get(key);
  return result?.[key];
}

async function _cwaGeminiStorageSet(key, value) {
  await chrome.storage.local.set({ [key]: value });
}

async function _cwaGeminiStoredRoutes() {
  const value = await _cwaGeminiStorageGet(CWA_GEMINI_ROUTE_MAP_KEY);
  return value && typeof value === "object" && !Array.isArray(value) ? value : {};
}

async function _cwaGeminiStoreRoute(conversationId, url) {
  if (!_cwaGeminiIsUrl(url)) throw new Error("GEMINI_ROUTE_IDENTITY_INVALID");
  const routes = await _cwaGeminiStoredRoutes();
  routes[conversationId] = url;
  await _cwaGeminiStorageSet(CWA_GEMINI_ROUTE_MAP_KEY, routes);
}

async function _cwaGeminiRouteForConversation(conversationId) {
  const routes = await _cwaGeminiStoredRoutes();
  const value = routes?.[conversationId];
  return typeof value === "string" && _cwaGeminiIsUrl(value) ? value : null;
}

async function _cwaGeminiExistingTab() {
  const stored = await _cwaGeminiStorageGet(CWA_GEMINI_RUNTIME_TAB_KEY);
  if (!Number.isInteger(stored)) return null;
  try {
    const tab = await chrome.tabs.get(stored);
    return _cwaGeminiIsUrl(tab?.url || "") ? tab : null;
  } catch {
    return null;
  }
}

async function _cwaGeminiEnsureTab(conversationId) {
  let targetUrl = CWA_GEMINI_ORIGIN + "/";
  if (conversationId !== null) {
    const storedRoute = await _cwaGeminiRouteForConversation(conversationId);
    if (storedRoute === null) throw new Error("GEMINI_CONTINUATION_ROUTE_UNKNOWN");
    targetUrl = storedRoute;
  }

  let tab = await _cwaGeminiExistingTab();
  if (tab === null) {
    tab = await chrome.tabs.create({ url: targetUrl, active: false });
    if (!Number.isInteger(tab?.id)) throw new Error("GEMINI_RUNTIME_TAB_CREATE_FAILED");
    await _cwaGeminiStorageSet(CWA_GEMINI_RUNTIME_TAB_KEY, tab.id);
  ...
  if (!_cwaGeminiIsUrl(tab?.url || "")) throw new Error("GEMINI_RUNTIME_TAB_ORIGIN_CHANGED");
  return tab;
}

function _cwaGeminiComposerExpression(text) {
  ...
  return "(() => {" +
    ...
    "const candidates=Array.from(document.querySelectorAll(" +
      "'div.ql-editor[contenteditable=\"true\"],rich-textarea [contenteditable=\"true\"],[aria-label=\"Enter a prompt here\"],[contenteditable=\"true\"][role=\"textbox\"],textarea'" +
    ...
    "}else{" +
      ...
      "for(const line of requestedText.split('\\n')){" +
        "const paragraph=document.createElement('p');" +
        "if(line){paragraph.textContent=line;}else{paragraph.appendChild(document.createElement('br'));}" +
...
function _cwaGeminiSnapshotExpression(promptText) {
  ...
  return "(() => {" +
    ...
    "const visible=(element)=>{" +
      ...
      "return rect.width>0&&rect.height>0&&style.display!=='none'&&style.visibility!=='hidden';" +
    "};" +
    "const selectorGroups=[[\"model-response\"],[\"message-content\"],[\".model-response-text\",\".response-content\",\"[data-message-author='assistant']\"],[\"main [class*='response']\",\"main [class*='markdown']\",\"main [class*='message']\",'main p','main pre']];" +
    ...
    "const generating=Array.from(document.querySelectorAll('button,[role=button]')).some((element)=>{" +
      ...
      "return label.includes('stop')||label.includes('cancel')||label.includes('останов')||label.includes('прер')||label.includes('停止')||label.includes('中止');" +
    "});" +
    "return {url:location.href,texts:texts.slice(-80),generating};" +
  "})()";
}

async function _cwaGeminiEvaluate(debuggee, expression) {
  const result = await _cwaBaseSendCommand(debuggee, "Runtime.evaluate", {
    expression,
    returnByValue: true,
    awaitPromise: true
  });
  if (result?.exceptionDetails) throw new Error("GEMINI_PAGE_EVALUATION_FAILED");
  return result?.result?.value;
}

async function _cwaGeminiWaitForComposer(debuggee, deadlineAt) {
  while (performance.now() < deadlineAt) {
    const snapshot = await _cwaGeminiEvaluate(debuggee, _cwaGeminiComposerExpression());
    if (snapshot?.found === true) return;
    await sleep(250);
  }
  throw new Error("GEMINI_COMPOSER_NOT_READY");
}

function _cwaGeminiSubmitExpression() {
  return "(() => {" +
    "const visible=(element)=>{" +
      "if(!(element instanceof Element))return false;" +
      "const rect=element.getBoundingClientRect();" +
    ...
    "const controls=Array.from(scope.querySelectorAll('button,[role=button],.send-button')).filter((element)=>visible(element)&&!disabled(element));" +
    ...
    "const semantic=controls.find((element)=>{" +
      ...
      "return label==='send'||label.includes('send message')||label.includes('submit')||label.includes('отправ')||label.includes('发送');" +
    "});" +
...
function _cwaGeminiDebuggerDetached(error) {
  const message = String(error?.message || error || "");
  return message.includes("Debugger is not attached to the tab");
}

async function _cwaGeminiReattachForObservation(debuggee, deadlineAt) {
  while (performance.now() < deadlineAt) {
    try {
...
async function _cwaGeminiSubmitOnce(debuggee, text, deadlineAt) {
  ...
  while (performance.now() < deadlineAt) {
    try {
      const submit = await _cwaGeminiEvaluate(debuggee, _cwaGeminiSubmitExpression());
      if (submit?.submitted === true) return;
    } catch (error) {
      if (_cwaGeminiDebuggerDetached(error)) {
  ...
  throw new Error("GEMINI_SUBMIT_CONTROL_NOT_READY");
}

function _cwaGeminiLatestNewText(snapshot, baseline) {
...
async function _cwaGeminiWaitForFinalText(debuggee, text, baseline, deadlineAt) {
  let lastText = null;
  let stableSince = null;
  while (performance.now() < deadlineAt) {
    let snapshot;
    try {
      snapshot = await _cwaGeminiEvaluate(debuggee, _cwaGeminiSnapshotExpression(text));
    } catch (error) {
      if (!_cwaGeminiDebuggerDetached(error)) throw error;
      await _cwaGeminiReattachForObservation(debuggee, deadlineAt);
      await sleep(250);
      continue;
    }
    const candidate = _cwaGeminiLatestNewText(snapshot, baseline);
    ...
    if (candidate !== null && snapshot?.generating !== true &&
        stableSince !== null && performance.now() - stableSince >= CWA_GEMINI_STABLE_MS) {
...
async function _cwaGeminiHandleTurn(message) {
  if (message?.type !== "turn") throw new Error("GEMINI_TURN_TYPE_REQUIRED");
  if (typeof message?.text !== "string" || !message.text.trim()) throw new Error("GEMINI_TEXT_REQUIRED");
  if ((Array.isArray(message?.attachmentPaths) && message.attachmentPaths.length > 0) ||
    ...
    throw new Error("GEMINI_MINIMAL_PROOF_UNSUPPORTED_OPTION");
  }
  ...
  const tab = await _cwaGeminiEnsureTab(conversationId);
  const initialUrl = tab.url || CWA_GEMINI_ORIGIN + "/";
  ...
  try {
    ...
    attached = true;
    await _cwaBaseSendCommand(debuggee, "Runtime.enable");
    await _cwaGeminiWaitForComposer(debuggee, deadlineAt);

    const before = await _cwaGeminiEvaluate(debuggee, _cwaGeminiSnapshotExpression(message.text));
...
registerProductProviderTurnHandler(
  CWA_GEMINI_PROVIDER_ID,
  _cwaGeminiHandleTurn
);
