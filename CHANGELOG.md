...
- final review transaction safety (Issue #169): journal schema 3 replaces the coarse BOUND/WRITE_ACKNOWLEDGED states with a persisted, transition-table-guarded machine (`PREPARED -> SUBMIT_DELEGATED -> WRITE_CONFIRMED -> RESPONSE_WAIT -> RESPONSE_CONFIRMED`, failure lane `WRITE_FINALITY_UNKNOWN -> RECONCILING -> WRITE_FOUND | NO_WRITE_PROVEN | AMBIGUOUS`); the delegation is write-ahead-recorded BEFORE the command leaves the process, any post-delegation loss lands in `WRITE_FINALITY_UNKNOWN` and MUST reconcile on journal + conversation/turn + history evidence before the single bounded retry edge, which only opens from a persisted `NO_WRITE_PROVEN` (`AMBIGUOUS` is terminal fail-closed); identity v3 binds the immutable 7-tuple (repo, issue, headSha, reviewAttemptId, pre-submit expected conversation or the `NO_PREASSIGNED_CONVERSATION` sentinel, promptHash, evidenceDigest) with `canonicalRequestId` self-consistency enforced at the storage chokepoint so post-write observed conversation/turn ids are correlated evidence, never a recomputed identity; the strong write-ACK tier that terminalizes `WRITE_CONFIRMED` requires EXACT non-empty submission-id correlation (foreign or missing-id write-completed events are evidence-only and fall to UNKNOWN; the typed SSE identity authority is read only from the exactly-correlated event), `WRITE_FOUND` requires the exact prompt/turn located via canonical/history evidence while an observed `conversationId` or bridge network evidence without a located turn is `AMBIGUOUS` (never write proof), and `NO_WRITE_PROVEN` opens only on a conclusive clean scan with no positive evidence - weak/correlated ACK observations (the ACK-reported conversation, uncorrelated write-completed frames, bridge network observations) are persisted to the durable journal before the UNKNOWN transition so a restart reconcile treats them as positive evidence and fails closed to `AMBIGUOUS` instead of losing them; the native bridge now surfaces
 `DELEGATION_ACCEPTED` as its own durable phase frame, separate from the write ACK and the canonical-readback response
- final review transport: add the production `FinalReviewTransport` over the browser-owned CWA plane with deterministic request binding (repository, issueNumber, pullRequestNumber, exact headSha, evidence digest -> canonical request identity) persisted durably before any external mutation; exactly one live write with the typed request-bound SSE conversation-identity authority required (absent authority or route-only `WEB:` identity fails closed), durable ACK persisted before finality, zero blind retry, post-write canonical read/reconcile with strict verbatim payload parse, exact response binding and unambiguous finality, and crash/restart reconciliation that never resends; replay, duplicate active submission, stale head, wrong target, malformed verdict, ambiguous finality and response mismatch all fail closed with stable codes
- browser-native boundary: propagate the request-bound SSE conversation-identity authority as typed fields (`sseConversationIdentityAuthority`, record/distinct counts) through `BrowserNativeTurnResult` and the `browser_native_write_completed` event so consumers can require the authority instead of trusting promoted ids; production-like E2E evidence additionally root-caused a stale `Service Worker/ScriptCache` snapshot that kept the pre-authority service worker running across browser restarts (operational fix: clear the profile `Service Worker` cache after extension source changes)
