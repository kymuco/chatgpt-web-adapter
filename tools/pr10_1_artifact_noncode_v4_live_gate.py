...
def _safe_candidate(value: Any) -> dict[str, Any] | None:
    ...
    return {
        "index": _safe_count(value.get("index"), maximum=24),
        ...
        "attribute_names": _safe_name_list(value.get("attributeNames"), max_items=64),
        ...
        "react_component_names": _safe_name_list(value.get("reactComponentNames"), max_items=48),
        "artifact_like_react_component_names": _safe_name_list(
            value.get("artifactLikeReactComponentNames"), max_items=48
        ),
        "candidate_reason_kinds": _safe_name_list(
            value.get("candidateReasonKinds"), max_items=16
        ),
    }


class ProductArtifactNonCodeV4Provider(ProductArtifactLiveProvider):
    """Bounded no-write provider for PR10.1 non-code assistant-turn structure reads."""
    ...
    def noncode_support(
        ...
        *,
        ...
        response, diagnostic = self._noncode_rpc(
            "characterizeGeneratedArtifactNonCodeV4Support",
            timeout=timeout,
        )
        fields = (
            "generatedArtifactNonCodeV4CharacterizationSupported",
            "generatedArtifactNonCodeV4CharacterizationSchemaVersion",
        ...
        support = {
            "supported": response.get("generatedArtifactNonCodeV4CharacterizationSupported") is True,
            "schema": response.get("generatedArtifactNonCodeV4CharacterizationSchemaVersion"),
            ...
            "pre_code_excluded": response.get("preCodeExcluded"),
    ...
    def noncode_snapshot(
        self,
        *,
        timeout: float = 10.0,
    ) -> tuple[dict[str, Any] | None, dict[str, Any]]:
        response, diagnostic = self._noncode_rpc(
            "characterizeGeneratedArtifactNonCodeV4",
            timeout=timeout,
        )
        fields = (
            "schema",
        ...
        snapshot = {
            ...
            "scanned_non_code_element_count": _safe_count(
                response.get("scannedNonCodeElementCount"), maximum=2048
            ),
            "structural_candidate_count": _safe_count(
                response.get("structuralCandidateCount"), maximum=24
            ),
            "identity_candidate_count": _safe_count(
                response.get("identityCandidateCount"), maximum=24
            ),
            "artifact_keyword_candidate_count": _safe_count(
                response.get("artifactKeywordCandidateCount"), maximum=24
            ),
            "locator_only_candidate_count": _safe_count(
                response.get("locatorOnlyCandidateCount"), maximum=24
            ),
        ...
        contract_ok = bool(
            snapshot["schema"] == NONCODE_SCHEMA
            and snapshot["pre_code_excluded"] is True
...
def run_gate(
    *,
    ...
    report: dict[str, Any] = {
        "schema": "CWA_PR10_1_ARTIFACT_NONCODE_V4_LIVE_GATE_V1",
        "noncode_schema": NONCODE_SCHEMA,
    ...
    try:
        support, diagnostic = provider.noncode_support(timeout=min(timeout, 10.0))
    except Exception as exc:
        report["support_probe_error_type"] = type(exc).__name__
        report["preflight_error"] = "ARTIFACT_NONCODE_V4_SUPPORT_RPC_FAILED"
        return report
    report["noncode_support"] = support
    report["support_probe_diagnostic"] = diagnostic
    if support != _EXPECTED_SUPPORT:
        report["preflight_error"] = "ARTIFACT_NONCODE_V4_SUPPORT_NOT_PROVEN"
        return report
    report["support_probe_proven"] = True

    if preflight_only:
        report["characterization"] = "ARTIFACT_NONCODE_V4_SUPPORT_PREFLIGHT_ONLY_PROVEN"
        report["ok"] = True
        return report

    report["surface_read_attempted"] = True
    try:
        snapshot, snapshot_diagnostic = provider.noncode_snapshot(timeout=min(timeout, 20.0))
    except Exception as exc:
        report["surface_read_error_type"] = type(exc).__name__
        return report
    report["noncode_snapshot"] = snapshot
    report["noncode_snapshot_diagnostic"] = snapshot_diagnostic
    if snapshot is None or snapshot_diagnostic.get("snapshot_contract_ok") is not True:
        report["characterization"] = "ARTIFACT_NONCODE_V4_SNAPSHOT_CONTRACT_NOT_PROVEN"
        return report
    ...
    structure_observed = bool(
        placement_proven and snapshot["structural_candidate_count"] >= 1 and candidates
    )
    identity_key_names_observed = any(
        item["identity_like_react_prop_names"] for item in candidates
    )
    artifact_structural_key_names_observed = any(
        item["artifact_like_attribute_names"]
        or item["artifact_like_react_prop_names"]
        or item["artifact_like_react_component_names"]
        or item["download_attribute_present"]
        for item in candidates
    )
    locator_only_structure_observed = bool(
        structure_observed
        and not identity_key_names_observed
        and not artifact_structural_key_names_observed
        and any(
            item["locator_like_react_prop_names"] or item["href_attribute_present"]
            for item in candidates
        )
    )

    report["experiment_valid"] = placement_proven
    report["noncode_structure_observed"] = structure_observed
    report["identity_key_names_observed"] = identity_key_names_observed
    report["artifact_structural_key_names_observed"] = artifact_structural_key_names_observed
    report["locator_only_structure_observed"] = locator_only_structure_observed

    if not placement_proven:
        report["characterization"] = "PROBE_TURN_PLACEMENT_NOT_PROVEN"
    elif not structure_observed:
        report["characterization"] = "PROBE_TURN_PROVEN_NO_NONCODE_ARTIFACT_STRUCTURE_OBSERVED"
    elif identity_key_names_observed:
        report["characterization"] = "PROBE_ANCHORED_NONCODE_IDENTITY_KEY_NAMES_OBSERVED"
    elif artifact_structural_key_names_observed:
        report["characterization"] = "PROBE_ANCHORED_NONCODE_ARTIFACT_STRUCTURAL_KEY_NAMES_OBSERVED"
    elif locator_only_structure_observed:
        report["characterization"] = "PROBE_ANCHORED_NONCODE_LOCATOR_ONLY_STRUCTURE_OBSERVED"
    else:
        report["characterization"] = "PROBE_ANCHORED_NONCODE_STRUCTURAL_CANDIDATES_OBSERVED"
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Run PR10.1 bounded no-write non-code artifact structure characterization."
    )
    ...
    if not args.preflight_only and not args.acknowledge_live_read:
        parser.error(
            ...
            "the gate performs one read-only non-code frontend snapshot, zero product writes, "
