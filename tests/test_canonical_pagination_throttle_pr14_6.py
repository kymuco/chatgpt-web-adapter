...
def test_only_idempotent_canonical_reads_receive_retry_and_pacing() -> None:
    ...
    assert "const fetchBytes = async (url, authorizeCurrent = false, retryThrottle = false)" in source
...
def test_throttle_exhaustion_never_claims_partial_history_complete() -> None:
    ...
    assert "const mergedPayload = { ...first.payload, messages: mergedMessages };" in source
