...
def _run_parser_cases() -> dict[str, object]:
    ...
    parser_start = schema29.index(
        "function _pr92Schema29ExtractRequestBoundConversationMetadata"
    )
    parser_end = schema29.index("extractSafeStreamMetadata = function", parser_start)
    parser = schema29[parser_start:parser_end]
...
const conflict = `data: {{"type":"message","conversation_id":"${{cid}}"}}\ndata: {{"type":"message","conversation_id":"${{other}}"}}\n`;
const encoded = Buffer.from(messageOnly, "utf8").toString("base64");
...
def test_schema_29_accepts_exact_request_top_level_id_without_stream_handoff():
    results = _run_parser_cases()
    expected = "11111111-2222-3333-4444-555555555555"
    for key in ("resumeOnly", "messageOnly", "base64"):
        parsed = results[key]
        assert parsed["conversationId"] == expected
        assert parsed["diagnostics"]["topLevelConversationIdRecordCount"] == 1
        assert parsed["diagnostics"]["distinctTopLevelConversationIdCount"] == 1
...
def test_schema_29_preserves_old_stream_handoff_as_consistent_special_case():
    ...
    assert parsed["diagnostics"]["topLevelConversationIdRecordCount"] == 2
    assert parsed["diagnostics"]["distinctTopLevelConversationIdCount"] == 1
    assert parsed["diagnostics"]["streamHandoffCount"] == 1


def test_schema_29_nested_id_has_zero_authority_and_conflicting_top_level_ids_fail_closed():
    results = _run_parser_cases()
    nested = results["nestedOnly"]
    assert nested["conversationId"] is None
    assert nested["diagnostics"]["topLevelConversationIdRecordCount"] == 0

    conflict = results["conflict"]
    assert conflict["conversationId"] is None
    assert conflict["diagnostics"]["distinctTopLevelConversationIdCount"] == 2
    assert conflict["diagnostics"]["conflictingConversationIds"] is True


def test_schema_29_parser_uses_only_top_level_exact_request_fields():
    ...
    assert "payload.conversation_id" in block
...
def test_schema_29_overwrites_schema_19_authority_name_and_keeps_route_diagnostic_only():
    ...
    assert "nestedConversationIdCanSatisfyIdentity: false" in text
...
def test_schema_29_support_gate_preserves_schema_28_and_requires_consensus_contract():
    ...
    assert "nested_conversation_id_can_satisfy_identity" in text
