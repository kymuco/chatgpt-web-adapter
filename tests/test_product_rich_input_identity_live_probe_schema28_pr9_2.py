...
PROBE = (
    ...
    / "src"
    / "chatgpt_web_adapter"
    / "product_rich_input_identity_live_probe_schema28_pr9_2.py"
)
...
def test_schema28_identity_probe_has_hard_one_write_budget_and_explicit_opt_in():
    text = _source()
...
def test_schema28_identity_probe_contains_exactly_one_product_send_call():
...
def test_schema28_identity_probe_is_image_new_chat_only():
    text = _source()
...
def test_schema28_identity_probe_requires_support_and_nonempty_resolved_identity():
    text = _source()
    assert "ProductRichInputSchema28LiveProvider" in text
    assert "_validate_support(support)" in text
    assert 'turn.get("conversation_id")' in text
    assert "PR9_2_SCHEMA28_IDENTITY_PROBE_CONVERSATION_ID_NOT_PROVEN" in text
...
def test_schema28_identity_probe_reuses_attachment_dependent_canonical_validator():
    text = _source()
    assert "_v7._validate_execution(" in text
    assert 'label="SCHEMA28_IDENTITY_IMAGE_NEW_CHAT"' in text
