...
def test_accepted_early_boundary_skips_post_network_waits() -> None:
    ...
    fallback_start = source.index(
        "if (diagnostics.earlyCompletionAccepted !== true)", candidate_start
    )
...
def test_early_repair_requires_conjunctive_current_answer_terminal_proof() -> None:
    ...
    assert (
        "finishReason !== null && finishCurrent && (endTurnCurrent || isCompleteCurrent)"
        in source
    )
...
def test_repair_installs_signal_only_around_current_page_turn() -> None:
    ...
    block = source[start:end]
    assert (
        "_cwaInstallOfficialPageEarlyCompletionSignal(active.terminalPromise)" in block
    )
