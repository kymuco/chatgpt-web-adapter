...
def test_schema_6_rich_submit_uses_page_deadline_guard_not_raw_cdp_input():
    ...
    assert (
        'PR92_PAGE_GUARDED_SUBMIT_PRIMITIVE = "PAGE_DEADLINE_GUARDED_SEND_BUTTON_CLICK"'
        in text
    )
    ...
    guarded_submit = text[
        text.index("async function _pr92ClosurePageDeadlineGuardedSubmit(") :
    ]
    guarded_submit = guarded_submit.split(
        "async function _executeNativeTurnWithPr92ClosureRepair", 1
    )[0]
...
def test_schema_6_attachment_count_requires_page_owned_composer_evidence_after_readiness():
    ...
    assert (
        'PR92_PAGE_ATTACHMENT_EVIDENCE_SOURCE = "PAGE_OWNED_COMPOSER_ATTACHMENT_STATE"'
        in text
    )
    ...
    submit = text.index("async function _pr92ClosurePageDeadlineGuardedSubmit(")
    ...
    assert (
        submit < pre_submit_evidence < send_ready < post_readiness_evidence < page_click
    )
