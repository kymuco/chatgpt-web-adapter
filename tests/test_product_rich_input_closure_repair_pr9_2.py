...
def test_schema_5_closure_overlay_is_loaded_last():
...
def test_schema_5_rich_submit_uses_page_deadline_guard_not_raw_cdp_input():
    text = CLOSURE.read_text(encoding="utf-8")

    assert "const PR92_CLOSURE_REPAIR_SCHEMA = 5;" in text
    ...
    guarded_submit = text[text.index("submitOfficialPageTurn = async function _pr92ClosurePageDeadlineGuardedSubmit") :]
    assert 'Input.dispatchMouseEvent' not in guarded_submit.split("executeNativeTurn = async function", 1)[0]
    assert 'Input.dispatchKeyEvent' not in guarded_submit.split("executeNativeTurn = async function", 1)[0]


def test_schema_5_attachment_count_requires_page_owned_composer_evidence():
    text = CLOSURE.read_text(encoding="utf-8")
    ...
    submit = text.index("submitOfficialPageTurn = async function _pr92ClosurePageDeadlineGuardedSubmit")
    pre_submit_evidence = text.index("_pr92ClosureWaitForPageOwnedAttachmentEvidence(", submit)
    page_click = text.index("_pr92ClosurePageGuardedSubmitExpression(", pre_submit_evidence)
    assert submit < pre_submit_evidence < page_click
