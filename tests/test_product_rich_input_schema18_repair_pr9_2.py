...
def test_schema_18_identity_resolution_is_deadline_bounded_and_has_no_write_primitive():
    ...
    end = text.index("async function _pr92Schema18ExecuteOfficialPageTurnWithIdentityAuthority", start)
    block = text[start:end]
...
def test_schema_18_success_requires_real_identity_after_write_completion_proof():
    text = SCHEMA18.read_text(encoding="utf-8")
    start = text.index("async function _pr92Schema18ExecuteOfficialPageTurnWithIdentityAuthority")
...
def test_schema_18_unresolved_identity_is_explicit_committed_failure_not_success():
    ...
    resolver_end = text.index(
        "async function _pr92Schema18ExecuteOfficialPageTurnWithIdentityAuthority", resolver_start
    )
