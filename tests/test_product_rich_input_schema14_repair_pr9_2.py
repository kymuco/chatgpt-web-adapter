...
def test_schema_14_guard_is_specific_to_new_composition():
    ...
    assert (
        "Text-only model-profile turns and ordinary rich-input turns are unchanged."
        in text
    )
...
def test_schema_14_gate_requires_composition_boundary_and_preserves_schema_13():
    ...
    assert (
        'support.get("rich_input_model_profile_combination_supported") is not False'
        in text
    )
    required = [
...
def test_schema_14_support_probe_is_eighth_no_write_characterization_rpc():
    text = GATE14.read_text(encoding="utf-8")
    assert (
        "This eighth characterization-only RPC carries neither text nor paths." in text
    )
    marker = '"characterizeRichInputSupport": True'
    assert marker in text
    request_block = text[
        text.index("request_id = str(uuid.uuid4())") : text.index(
            "if response.get", text.index("request_id = str(uuid.uuid4())")
        )
    ]
